export async function onRequest(context) {
  ...
  try {
    // 1. Fetch Peptides + FAQs linked to them
    const peptidesRes = await env.DB.prepare(`
      SELECT 
        p.*, 
    ...
    // 2. Fetch Stacks + FAQs linked to them
    const stacksRes = await env.DB.prepare(`
      SELECT 
        s.*, 
        GROUP_CONCAT(f.question, '|||') as faq_questions,
        GROUP_CONCAT(f.answer, '|||') as faq_answers
    ...
    return new Response(JSON.stringify({
      peptides: peptidesRes.results,
      stacks: stacksRes.results
    }), {
      headers: { 
